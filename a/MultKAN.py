...
def Interpolate_error_bounds(kx, x, k):            
    prod_diff = []
    for x0 in x:
        abs_diff = np.abs(kx - x0)
        idx = np.argsort(np.abs(abs_diff))
        prod_diff.append( np.prod(abs_diff[idx[:k]]) )
    
    intrpopl = np.array(prod_diff)    
    return np.abs( intrpopl)


def Error_at_knots_bounds(kx, ky, x, k):        
    res = []
    for x0 in x:
        abs_diff = np.abs(kx - x0)
        idx = np.argsort(np.abs(abs_diff))
    
        # ii, jj = find_closest_range(er[idx], x0, k)    
        coef = div_diff(kx[idx],ky[idx],k)
        # print('coef',len(coef), ii)

        res0 = Eval(coef, 0, kx[idx], x0, k)            
        res.append(res0)
    knot_er = np.array(res).flatten()
    return np.abs(knot_er)

def calculate_error_bounds(kx, ky, x, k, fk = 1.0, ):        
    res = []
    prod_diff = []
    for x0 in x:
        abs_diff = np.abs(kx - x0)
        idx = np.argsort(np.abs(abs_diff))
        prod_diff.append( fk * np.prod(abs_diff[idx[:k]]) / np.math.factorial(k))
    ...
    return np.abs( intrpopl), np.abs(knot_er)
    
def find_closest_range(Tii, x0 = 0, k = 4):        
    # Tii.copy().sort()
    a,b = min(Tii), max(Tii)    
    if k > len(Tii):
        k = len(Tii)
...
def div_diff(x,y, k = 3):
    diff = [y]
    for k_ in range(1,k):        
        # print(k_, diff[-1][1:] - diff[-1][:-1], x[k_:]-x[:-k_])
        diff.append((diff[-1][1:] - diff[-1][:-1])/(x[k_:]-x[:-k_]))
    return diff

def Eval(coef, ii, g, x, k = 3):        
    gg = g[ii:ii+k].reshape((1,-1))    
    t = x.reshape((-1,1))
    sp = coef[0][ii] + np.sum([coef[k_][ii] * np.prod( (t-gg)[:,:k_],axis = 1) for k_ in range(1,k)] ,axis=0)

    return sp

def sp_er2(xk, yk, x0, k= 3, abs = True):
    indx = np.argsort(xk)
    xk = xk[indx]
    yk = yk[indx]
    ii, jj = find_closest_range(xk, x0, k)    
    coef = div_diff(xk,yk,k)
    # print('coef',len(coef), ii)
    res0 = Eval(coef, ii, xk, x0, k)
    if abs:
        return np.abs(res0)
    return res0

def sp_er3(xk, x0, k=3,fk = 1):
    indx = np.argsort(xk)
    xk = xk[indx]
    # yk = yk[indx]    
    ii, jj = find_closest_range(xk, x0, k+1)
    gg = xk[ii:ii+k+1].reshape((1,-1))    
    t = x0.reshape((-1,1))
    sp = fk * np.prod( (t-gg),axis = 1) / np.math.factorial(k+1)
    return np.abs(sp)

def sp_err(xk, yk, Xs, k, fk = 1):
    res = []
    for x0 in Xs:
        x0 = x0.numpy()
        res0 = sp_er2(xk, yk, x0, k= k)
        res1 = sp_er3(xk, x0, k= k,fk=fk)
        res.append([res0,res1])

    res = np.array(res)
    return (res[:,1]+res[:,0])[:,0]

def sp_err_det(xk, yk, Xs, k, fk = 1):
    res = []
    for x0 in Xs:
        x0 = x0.numpy()
        # print(xk,yk,x0)
...
def ErrX(Tii, fk = 1, x0 = 0, k = 4):    
    """
    It  calculates the error at input x0. 
    Tii: list of knots
    fk: Lipschitz constant of k's derivative of f.
    x0: test points.
    k: order of spline. (k=4 cubic spline)
    ...
    Tii.copy().sort()  
    # print(1,Tii)  
    ii, jj = find_closest_range(Tii, x0, k)        
    prod = np.prod(Tii[ii:jj+1] -x0)
    # print(Tii[ii:jj+1] ,x0,prod)
    return np.abs(fk * prod / np.math.factorial(k))

def ErrXs(Tii, fk = 1, x = [], k = 4):   
    """
    It  calculates the error bound for a list of inputs. 
    Tii: list of knots
    fk: Lipschitz constant of k's derivative of f.
    x: list of test points.
    k: order of spline. (k=4 cubic spline)
    
    return err
    err: list of error values.
    """ 
    return np.array([ErrX(Tii, fk, x0, k) for x0 in x])
...
class MultKAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0, 
        ...
        for l in range(self.depth):
            ...
            '''
            sp_batch = KANLayer(in_dim=width_in[l], out_dim=width_out[l+1], num=grid, k=k, 
                                noise_scale=noise_scale, scale_base_mu=scale_base_mu, 
                                scale_base_sigma=scale_base_sigma, scale_sp=1.,
    ...
    def forward_l(self, x, L, singularity_avoiding=False, y_th=10.):        
        x = x[:,self.input_id.long()]
        assert x.shape[1] == self.width_in[0]
    ...
    def sp_err(self, Xs, k, fk = 1):
        xg,yg,ygt = self.knots['x0'],self.knots['y_hat'],self.knots['y_true']
        indx0 = xg.flatten().argsort()
        xk = xg.flatten()[indx0]
        yk = np.abs((yg-ygt).flatten().numpy())[indx0]
    ...
    def ErrL(self, l, i, j):
        """
        It  calculates the error at latest input that passed to network. 
        l: is layer
        i: is the input of layer
        j: output of the layer.
        * layer has i*j spline, so i, j  is the address of spline in layer.

        return x,y,g,gx,err
        x: input  to this splie
        y: output of this splie
        gx: knots  of this splie
        gy: knots values of this splie
        err: error bound of this splie
        """
        if not 'errl' in self.__dir__():
            self.errl = {}
        k = self.k + 1
        rank = torch.argsort(self.acts[l][:, i])
        act = self.act_fun[l]
        inp = self.act_fun[l].in_dim
        out = self.act_fun[l].out_dim
        
        kl = self.act_fun[l].k
        gx = self.act_fun[l].grid[i:i+1,:].T
        # gx = act(g.T)[2][:,j,i].cpu().detach().numpy()
        # print(g.shape,l,i,j)
        gy = act(gx)[2][:,j,i].cpu().detach().numpy()
        gy = gy * self.node_scale[l].detach().numpy()[j] + self.node_bias[l].detach().numpy()[j]
        # gy = (act(g.T)[2][:,:,i].cpu().detach().numpy() + \
        #    self.biases[l].weight.cpu().detach().numpy())[:,j]
        
        # b = self.biases[l].weight.detach().numpy()
        x  = self.acts[l][:, i:i+1][rank].cpu().detach().numpy()
        y  = self.spline_postacts[l][:, j, i][rank].cpu().detach().numpy() #+ b[0,j]
        # y  = self.acts[l+1][:, j][rank].cpu().detach().numpy() + b[0,j]
        y = y * self.node_scale[l].detach().numpy()[j] + self.node_bias[l].detach().numpy()[j]
        fk = self.fk[l]
        f1 = self.f1[l]
        gx = gx[:,0].cpu().detach().numpy()
        # print(g.shape, gx.shape)
        # print(x.shape, g.shape)
        # err_c = fk * np.prod(x - g, axis = 1) ## It uses all knots
        # print(x.shape, g.shape,err_c.shape)
        err_p = 0
        err_c = np.abs(ErrXs(gx, fk = fk, x = x, k = k)) # Error of this layer
        if l > 0: # Error of previous layers 
            Bs = self.width[l-1][0]
            err_ps = np.array([self.ErrL(l-1, b, i)[-1] for b in range(Bs)])
            # print(err_ps[-1].shape)
            err_p = np.abs( f1 * np.sum(err_ps, axis= 0))
        # self.errl[f'{l}-{i}-{j}'] = (gx,gy,x,y,err_p, err_c,err_c + err_p)
        self.errl[f'{l}-{i}-{j}'] = {'gx':gx,
                                     'gy':gy,
                                     'x':x,
                                     'y':y,
                                     'err_p':err_p, 
                                     ...
                                     'err_l':err_c + err_p,
                                     'rank' :rank}
        return x[:,0],y,gx,gy,err_c + err_p

    def ErrN(self):
        """
        It  calculates the Network error at latest input that passed to network,
        for corresponding j's.         
        
        return err
        err: list of error bound for each outputs
        """
        l = len(self.width) -1 
        Bs = self.width[l-1][0]
        J = self.width[-1][0]
        err_ps = [np.sum(np.array([self.ErrL(l-1, b, j)[-1] for b in range(Bs)]),axis = 0) for j in range(J)]
        return err_ps
    
    def DAREK_torch(self, x0, fk = 1.0, f1 = 1.0, share = None):        
        ...
        yhat = self(x0)
        if np.isscalar(fk):
            fk = [np.power(fk,1/depth) for l in range(depth)]    
        if np.isscalar(f1):
            f1 = [np.power(f1,1/depth) for l in range(depth)]    
        if share is None:
            s = 0
            for l in range(depth):    
    ...
    def DAREK(self, x0, fk = 1.0, f1 = 1.0, share = None):        
        assert 'samples' in self.__dir__(), 'Need to fit model with flage "nonfixknot=True"'
        self.splines = {'samples': self.samples}
        depth = len(self.width) - 1
        gout = self(self.samples['x'])
        ej = gout - self.samples['y']
        ...
        for l in range(depth):    
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l+1]):
                    # print(f'{l}-{i}-{j}')                
                    sp = {'L1': f1[l], 'Lk': fk[l]}
                    indx = self.knots[f'indx{l}'][:,i].numpy().copy()
                    sp['share'] = share[l].copy()
                    sp['ej']    = (ej[indx] * sp['share']).detach().cpu().clone()
                    sp['indx']  = indx.copy()
                    sp['gi']    = self.act_fun[l].grid[i].clone()           
                    ...
                    sp['hx'] = self.acts[l][:,i].clone()
                    sp['hy'] = self.spline_postacts[l][:,j,i].clone()              
                    
                    util = []
                    for o in range(self.width_out[depth]):
                        func = splrep(sp['gi'].clone(), sp['ej'][:,o:o+1].clone(), k=k)
                        yhat_hl = splev(sp['hx'], func)
                        util.append(np.abs(yhat_hl.copy()))
                    util = np.array(util).copy()
                    # breakpoint()
                    # print(l, sp['gi'])
                    
                    ubar = np.array([sp['Lk'] * np.prod( np.sort(np.abs(x_.reshape((-1,1)) - sp['gi'].numpy().reshape((1,-1))))[:,:kp] , axis = 1)/ kfac for x_ in sp['hx']])
                    # print(ubar.shape, ubar.T)
                    # print(util.shape, util.T)
                    sp['Ubar_c'] = ubar  .copy()
                    sp['Util_c'] = util  .copy()
                    if l > 0:  
                        # need to change it???              
                        sp['Util_p'] = np.sum([self.splines[f'{l-1}-{b}-{i}']['Util'] for b in Range( self.width_in[l-1])], axis=0).copy()
                        sp['Ubar_p'] = np.sum([self.splines[f'{l-1}-{b}-{i}']['Ubar'] for b in Range( self.width_in[l-1])], axis=0).copy()
                        sp['Ubar']   = sp['Ubar_c'] + sp['L1'] * sp['Ubar_p']
                        sp['Util']   = sp['Util_c'] + sp['L1'] * sp['Util_p']
                        # print('c',f'{l}-{i}-{j}',sp['Util_c'].shape, sp['Util_c'].T)
                        # print('p',f'{l}-{i}-{j}',sp['Util_p'].shape, sp['Util_p'].T)
        ...
        for i in range(self.width_out[depth]):
            sp = {}        
            sp['Ubar'] = np.sum([self.splines[f'{depth-1}-{b}-{i}']['Ubar'] for b in Range( self.width_in[depth-1])], axis=0)#[:,0]
            Et += np.sum([self.splines[f'{depth-1}-{b}-{i}']['Util'] for b in Range( self.width_in[depth-1])], axis=0)
            self.splines[f'U{i}'] = sp       
        ...
        return yhat.detach(), np.array([self.splines[f'U{i}']['U'] for i in range(self.width_out[-1])]).T
    ...
    def DAREK(self, x0, fk = 1.0, f1 = 1.0, share = None):        
            ...
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l+1]):
                    ...
                    for o in range(self.width_out[depth]):
                        func = splrep(sp['gi'].clone(), sp['ej'][:,o:o+1].clone(), k=k)
                        yhat_hl = splev(sp['hx'], func)
                        sp['etilde'].append(np.abs(yhat_hl.copy()))
                    sp['etilde'] = np.array(sp['etilde']).copy()
                    # breakpoint()
                    # print(l, sp['gi'])
                    ebar = np.array([sp['Lk'] * np.prod( np.sort(np.abs(x_.reshape((-1,1)) - sp['gi'].numpy().reshape((1,-1))))[:,:kp] , axis = 1)/ kfac for x_ in sp['hx']])
                    
                    sp['E_cb'] = ebar          .copy()
                    sp['E_ct'] = sp['etilde']  .copy()
                    if l > 0:                
                        sp['E_pt'] = np.sum([self.splines[f'{l-1}-{b}-{i}']['Et'] for b in Range( self.width_in[l-1])], axis=0)
                        sp['E_pb'] = np.sum([self.splines[f'{l-1}-{b}-{i}']['Eb'] for b in Range( self.width_in[l-1])], axis=0)
                        sp['Eb'] = sp['E_cb'] + sp['L1'] * sp['E_pb']
                        sp['Et'] = sp['E_ct'] + sp['L1'] * sp['E_pt']
                    else:
                        sp['E_pt'] = 0.0
                        sp['E_pb'] = 0.0
                        sp['Eb'] = sp['E_cb']
        ...
        Et = self.splines[f'0-0-0']['Et'] * 0.0
        for i in range(self.width_out[depth]):
            sp = {}        
            sp['Eb'] = np.sum([self.splines[f'{depth-1}-{b}-{i}']['Eb'] for b in Range( self.width_in[depth-1])], axis=0)#[:,0]
            Et += np.sum([self.splines[f'{depth-1}-{b}-{i}']['Et'] for b in Range( self.width_in[depth-1])], axis=0)
            self.splines[f'E{i}'] = sp
            
            
        
        for i in range(self.width_out[depth]):
            sp = self.splines[f'E{i}']        
            sp['Et'] = Et[i,:]
            sp['E'] = sp['Et'] + sp['Eb']
            
        return yhat.detach(), np.sum([self.splines[f'E{i}']['E'] for i in range(self.width_out[depth])], axis= 1).T
    """
    ...
    def DAREK(self, x0, fk = 1.0, f1 = 1.0, share = None):        
        ...
        if np.isscalar(share):
            share = [share for l in range(depth)]    
        ...
        for l in range(depth):    
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l+1]):
                    # print(f'{l}-{i}-{j}')                
                    sp = {'L1': f1[l], 'Lk': fk[l]}
                    indx = self.knots[f'indx{l}'][:,i]
                    sp['share'] = share[l]
                    sp['ej']    = (ej[indx] * sp['share']).detach().cpu()
                    sp['indx']  = indx
                    sp['gi']    = self.act_fun[l].grid[i].clone()           
                    if l+1 < depth:
                        sp['gj'] = self.act_fun[l+1].grid[j]                    
                    else:
                        sp['gj'] = (gout[j]).detach().cpu()
                    ...
                    for o in range(self.width_out[depth]):
                        func = splrep(sp['gi'], sp['ej'][:,o:o+1], k=k)
                        yhat_hl = splev(sp['hx'], func)
                        sp['etilde'].append(yhat_hl)
                    sp['etilde'] = np.array(sp['etilde'])
                    # breakpoint()
                    ebar = np.array([sp['Lk'] * np.prod( np.sort(np.abs(x_.reshape((-1,1)) - sp['gi'].numpy()))[:,:kp] , axis = 1)/ kfac for x_ in x0])
                    sp['ebar'] = ebar            
                    sp['E_c'] = ebar + sp['etilde']   
                    if l > 0:                
                        sp['E_p'] = np.sum([self.splines[f'{l-1}-{b}-{i}']['E'] for b in Range( self.width_in[l-1])], axis=0)
                        sp['E'] = sp['E_c'] + sp['L1'] * sp['E_p']
                    else:
                        sp['E_p'] = 0.0
                        sp['E'] = sp['E_c']
                    self.splines[f'{l}-{i}-{j}'] = sp
        ...
        for i in range(self.width_out[depth]):
            sp = {}
            sp['E'] = np.sum([self.splines[f'{depth-1}-{b}-{i}']['E'] for b in Range( self.width_in[depth-1])], axis=0)
            self.splines[f'E{i}'] = sp
        return yhat.detach(), np.sum([self.splines[f'E{i}']['E'] for i in range(self.width_out[depth])], axis= 0).T
    """
